...
def compute_single_source_shortest_paths(graph, source):
...
def initialize_heap(graph, source):
    """Initialize Min-Heap"""
    dijkstra_criterion = {key: sys.maxsize for key in graph if key != source}
    for node in graph[source]:
        dijkstra_criterion[node[0]] = node[1]

    return PriorityQueue([[value, key] for key, value in dijkstra_criterion.items()])
